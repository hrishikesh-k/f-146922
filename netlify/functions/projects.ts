...
exports.handler = async function (event: HandlerEvent) {
  ...
  switch (slug) {
    case 'main':
      const devSortedProjects = devProjects.sort((projectA, projectB) => projectB.year - projectA.year);
      return {
        statusCode: 200,
        headers: {
    ...
    case 'design':
      const designSortedProjects = designProjects.sort((projectA, projectB) => projectB.year - projectA.year);
      return {
        statusCode: 200,
        headers: {
          "Content-Type": "application/json",
    ...
    case 'intranet':
      const intranetSortedProjects = intranetProjects.sort((projectA, projectB) => projectB.year - projectA.year);
      return {
        statusCode: 200,
        headers: {
          "Content-Type": "application/json",
